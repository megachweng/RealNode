...
const char H3C_VERSION[16]="CH V5.10-0301"; // 华为客户端版本号
		...
		for (;;)
		{
			...
			{	// 处理认证失败信息
				...
				{	// 输出错误提示消息
					...
					// E63018用户不存在
					// E63032密码错误
                    if(strstr(msg,"E63018")){
                        printf("用户不存在");
                        system("osascript -e 'display notification \"用户不存在！\" with title \"RealNode提示\" sound name \"Basso.aiff\"'");
                        system("ps -ef | grep RealNode | grep -v grep | awk '{print $2}' | xargs kill -9");

                    }else if (strstr(msg, "E63032")){
                        printf("密码错误");
                        system("osascript -e 'display notification \"密码错误！\" with title \"RealNode提示\" sound name \"Basso.aiff\"'");
                        system("ps -ef | grep RealNode | grep -v grep | awk '{print $2}' | xargs kill -9");
                    }
